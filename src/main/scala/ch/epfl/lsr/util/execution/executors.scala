...
object Executors {
  ...
  def newCachedOrderedThreadPoolExecutor(keyFunction :Runnable=>Object, name :String) = { 
    new OrderedThreadPoolExecutor(0, Integer.MAX_VALUE, java.lang.Long.MAX_VALUE, TimeUnit.SECONDS, new LinkedBlockingQueue(1000), name) { 
  ...
  def newFixedThreadPoolExecutor(nthreads :Int, keyFunction :Runnable=>Object, name :String) = { 
    new OrderedThreadPoolExecutor(nthreads, nthreads, java.lang.Long.MAX_VALUE, TimeUnit.SECONDS, new LinkedBlockingQueue(1000), name) { 
      def getChildExecutorKey(task :Runnable) = keyFunction(task)
    }
  }
}
